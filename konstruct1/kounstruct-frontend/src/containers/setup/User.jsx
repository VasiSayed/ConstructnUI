...
function User() {
  ...
  const userRole = useMemo(() => {
    ...
    else if (userData.roles && userData.roles.length > 0) return userData.roles[0];
    ...
    if ((is_manager || canCreateManager) && org) {
      ...
      try {
        ...
        if ((is_manager || canCreateManager) && projects.length > 0) {
          ...
          setOrgManagerTypes(res.data.manager_types || ["Project Manager", "Site Manager"]);
        }
      } catch (err) {
  ...
  const handleOrganizationChange = useCallback(
    async (e) => {
      ...
      if (orgId && (is_manager || canCreateManager) && parseInt(orgId) !== parseInt(org)) {
        setProjectsLoading(true);
        try {
          ...
          <div className={`px-6 py-5 max-w-7xl mx-auto rounded ${palette.card} ${palette.cardBorder} ${palette.shadow}`}>
            ...
            <div className="mb-6 flex items-center justify-between">
              <div>
                ...
                <p className={`${palette.text} opacity-80 mt-2`}>User creation and management</p>
                <div className="mt-3">
                  <span className={`inline-block px-3 py-1 rounded-full text-sm ${palette.error}`}>
              ...
              <div className="text-center">
                <div className="mb-6">
                  ...
                  <p className={`${palette.text} opacity-70`}>
                    You do not have permissions to create users. Please contact your administrator.
                  </p>
                </div>
              </div>
            </div>
